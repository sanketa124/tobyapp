...
def preprocess_images(images):
    ...
    coords = torch.stack(torch.meshgrid(
        torch.linspace(0, 1, H, device=images.device),
        torch.linspace(0, 1, W, device=images.device),
        indexing='ij'  # 🔧 Add indexing to avoid warning
    ), dim=0)  # [2, 28, 28]

    coords = coords.reshape(2, -1).unsqueeze(0).repeat(B, 1, 1)  # [B, 2, 784]
...
if __name__ == "__main__":
    ...
    torch.save(model.cpu().state_dict(), "model.pth")  # Save model to CPU format